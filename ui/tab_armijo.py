...
def create_tab_armijo():
    ...
    btn = QPushButton("Ejecutar Método de Armijo")
    ...
    def ejecutar():
        ...
        try:
            ...
            result_label.setText(
                f"x* = {x_opt.round(5)},   f(x*) = {f_opt:.6f}"
            )
            ...
            canvas.ax.clear()
            canvas.ax.contour(X, Y, Z, levels=20, cmap="viridis")

            # Trayectoria
            canvas.ax.plot(xs, ys, "ro--", linewidth=2, label="Trayectoria")
            canvas.ax.scatter(x_opt[0], x_opt[1], c="red", s=60, label="x*")

            canvas.ax.set_title("Trayectoria del Descenso (Armijo)")
            canvas.ax.grid(True)
            canvas.ax.legend()
            ...
            for row, (k, xk, fx, alpha, gradn) in enumerate(hist):
                data = [
                    k,
                    np.round(xk, 4),
                    f"{fx:.6f}",
